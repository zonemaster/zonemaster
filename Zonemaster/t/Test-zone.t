...
my %res = map {$_->tag => 1} Giraffa->test_module(q{zone}, q{afnic.fr});
ok($res{MNAME_NO_RESPONSE}, q{SOA 'mname' noe response});
ok($res{MNAME_NOT_IN_GLUE}, q{SOA 'mname' not listed as NS});

%res = map {$_->tag => 1} Giraffa->test_module(q{zone}, q{add.tf});
...
%res = map {$_->tag => 1} Giraffa->test_module(q{zone}, q{google.tf});
