...
It is not enough to have a signed zone. The parent zone must have DS record to
...
Signing the zone and providing DS records for a signed zone is not reuired by
...
| DS07_DS_ON_PARENT_SERVER      | INFO    | ns_list        | The following parent name servers responds with DS record or records for the child zone. Servers: {ns_list}                                |
| DS07_INCONSISTENT_DS          | ERROR   |                | Inconsistent responses from parent name servers. Some includes DS, others do not.                                                          |
| DS07_INCONSISTENT_SIGNED      | ERROR   |                | Inconsistent responses from name server. Some include signed responses, others do not.                                                     |
...
| DS07_NOT_SIGNED_ON_SERVER     | INFO    | ns_list        | The following name servers responds with no DNSKEY (unsigned child zone). Servers: {ns_list}.                                              |
| DS07_NO_DS_ON_PARENT_SERVER   | INFO    | ns_list        | The follwing parent name servers responds without DS record for the child zone. Servers: {ns_list}.                                        |
