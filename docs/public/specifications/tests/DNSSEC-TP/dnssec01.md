...
|:-------------------------|:--------|:--------------------------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------|
| DS01_DS_ALGO_2_MISSING   | NOTICE  | ip_list, keytag                             | There is a DS record with keytag {keytag}. A DS record using digest algorithm 2 (SHA-256) is missing. Fetched from parent name servers (IP) "{ip_list}".     |
| DS01_DS_ALGO_DEPRECATED  | ERROR   | ip_list, keytag, ds_algo_num, ds_algo_descr | The DS record with keytag {keytag} uses a deprecated digest algorithm {ds_algo_num} ({ds_algo_descr}) on parent name servers (IP) "{ip_list}".               |
| DS01_DS_ALGO_NOT_DS      | ERROR   | ip_list, keytag, ds_algo_num, ds_algo_descr | The DS record with keytag {keytag} uses a digest algorithm {ds_algo_num} ({ds_algo_descr}) not meant for DS records on parent name servers (IP) "{ip_list}". |
| DS01_DS_ALGO_OK          | INFO    | ip_list, keytag, ds_algo_num, ds_algo_descr | The DS record with keytag {keytag} uses digest algorithm {ds_algo_num} ({ds_algo_descr}), which is OK. Fetch from parent name servers (IP) "{ip_list}".      |
| DS01_DS_ALGO_PRIVATE     | ERROR   | ip_list, keytag, ds_algo_num                | The DS record with keytag {keytag} uses a digest algorithm {ds_algo_num} for private use on parent name servers (IP) "{ip_list}".                            |
| DS01_DS_ALGO_RESERVED    | ERROR   | ip_list, keytag, ds_algo_num                | The DS record with keytag {keytag} uses a reserved digest algorithm {ds_algo_num} on name servers (IP) "{ip_list}".                                          |
| DS01_DS_ALGO_UNASSIGNED  | ERROR   | ip_list, keytag, ds_algo_num                | The DS record with keytag {keytag} uses a unassigned digest algorithm {ds_algo_num} on parent name servers (IP) "{ip_list}".                                 |
