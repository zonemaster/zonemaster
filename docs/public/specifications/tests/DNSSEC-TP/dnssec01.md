...
| DS01_DS_ALGO_PRIVATE     | ERROR   | ns_ip_list, keytag, ds_algo_num                | The DS record with keytag {keytag} uses a digest algorithm {ds_algo_num} for private use. Fetched from parent name servers (IP) "{ns_ip_list}".                                         |
...
| DS01_DS_ALGO_UNASSIGNED  | ERROR   | ns_ip_list, keytag, ds_algo_num                | The DS record with keytag {keytag} uses an unassigned digest algorithm {ds_algo_num} on parent name servers (IP) "{ns_ip_list}".                                            |
