...
This document has instructions for translation in PO files and the Gettext
system. It is used in Zonemaster-Engine, Zonemaster-CLI and Zonemaster-Backend.
Zonemaster-GUI uses a different solution. See the "[Translation Guide]" for
Zonemaster-GUI for translation instructions for that part.
...
and a fork of the Zonemaster repositories. If you do not have a Github account
you can easily create one at [Github]. If you are not prepared to
...
The steps in this section will work for most translation work. We welcome
comments on these. The steps are the same for all three repositories.
...
* When doing the update, do not change the *msgid*, only the *msgstr*. The
  *msgid* cannot and must not be be update in this process. They are the links
  between the Perl module and the *PO* file.
...
* With the following command, check that the messages arguments (`{...}`) in all
  *msgstr* strings match up with those in the *msgid* strings. This step should
  only be done in Zonemaster-Engine as it will only work and be relevant there.
  ```
...
* Select to create a new pull request. Using Zonemaster-Engine as an example,
  the base repository should be *zonemaster/zonemaster-engine* and the base
  branch should be *develop* (not *master*). The "head" should be your fork and
  "compare" the same branch as you created above and pushed to your fork,
  "translation-update".
...
that you feel is inferior then you can provide that information to the project.
The preferred way is to create an issue in the relevant repository.
...
* Determine what the language code of the new language should be. I must be
  ...
  should be.
  ```
  locale -a | grep xx_      # Works in FreeBSD
  grep xx_ /etc/locale.gen  # Works in Linux
  ```
...
* Now you go back to the translation steps and continue in the same was with an
  existing language.
...
[Tools]:                                       #tools
[Translation Guide]:                           https://github.com/zonemaster/zonemaster-gui/blob/develop/docs/TranslationGuide.md
