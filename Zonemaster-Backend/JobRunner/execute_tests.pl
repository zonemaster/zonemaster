...
do {
	...
	while (my $h = $sth1->fetchrow_hashref) {
		if (can_start_new_worker(10, $h->{id})) {
			my $command = "/home/toma/perl5/perlbrew/perls/perl-5.20.0/bin/perl /home/toma/PROD/zonemaster/Zonemaster-Backend/JobRunner/execute_zonemaster_P10.pl $h->{id} > /home/toma/LOGS/execute_zonemaster_P10_$h->{id}_$start_time.log 2>&1 &";
			say $command;
			system($command);
		}
	}
	...
	while (my $h = $sth1->fetchrow_hashref) {
		if (can_start_new_worker(5, $h->{id})) {
			my $command = "/home/toma/perl5/perlbrew/perls/perl-5.20.0/bin/perl /home/toma/PROD/zonemaster/Zonemaster-Backend/JobRunner/execute_zonemaster_P5.pl $h->{id} > /home/toma/LOGS/execute_zonemaster_P5_$h->{id}_$start_time.log 2>&1 &";
			say $command;
			system($command);
		}
	}
